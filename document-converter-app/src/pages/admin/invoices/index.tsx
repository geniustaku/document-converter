// pages/admin/invoices/index.tsx - Admin invoices list
      ...
      <div style={{ minHeight: '100vh', background: '#f4f4f4', display: 'flex' }}>
            ...
            {navItems.map((item) => (
              <Link
                ...
                style={{
                  ...
                  gap: '12px',
                  padding: '12px 24px',
                  color: item.id === 'invoices' ? '#ffffff' : '#959595',
                  background: item.id === 'invoices' ? 'rgba(250, 15, 0, 0.15)' : 'transparent',
                  borderLeft: item.id === 'invoices' ? '3px solid #fa0f00' : '3px solid transparent',
                  textDecoration: 'none',
                  fontSize: '14px',
                  fontWeight: '500',
                  transition: 'all 0.2s'
        ...
        <main style={{ flex: 1, marginLeft: '260px', padding: '32px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
            <div>
              <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#323232', margin: '0 0 4px 0' }}>Invoices</h1>
              <p style={{ fontSize: '14px', color: '#6e6e6e', margin: 0 }}>Manage all invoices</p>
            </div>
            <Link
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#fafafa', borderBottom: '1px solid #e5e5e5' }}>
                  <th style={{ padding: '14px 24px', textAlign: 'left', fontSize: '12px', fontWeight: '600', color: '#6e6e6e', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Invoice</th>
                  ...
                  <th style={{ padding: '14px 24px', textAlign: 'left', fontSize: '12px', fontWeight: '600', color: '#6e6e6e', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Amount</th>
                  ...
                  <th style={{ padding: '14px 24px', textAlign: 'left', fontSize: '12px', fontWeight: '600', color: '#6e6e6e', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Due Date</th>
              ...
              <tbody>
                {invoices.length > 0 ? (
                  invoices.map((invoice) => (
                    <tr key={invoice.id} style={{ borderBottom: '1px solid #f4f4f4', transition: 'background 0.2s' }}>
                      <td style={{ padding: '16px 24px' }}>
                        <Link href={`/admin/invoices/${invoice.id}`} style={{ fontSize: '14px', fontWeight: '600', color: '#fa0f00', textDecoration: 'none' }}>
                          {invoice.invoice_number}
                        </Link>
                        <p style={{ fontSize: '12px', color: '#959595', margin: '2px 0 0 0' }}>{formatDate(invoice.issue_date)}</p>
                      </td>
                      <td style={{ padding: '16px 24px' }}>
                        <p style={{ fontSize: '14px', color: '#323232', margin: 0 }}>{invoice.company_name}</p>
                      </td>
                      <td style={{ padding: '16px 24px' }}>
                        <p style={{ fontSize: '14px', fontWeight: '600', color: '#323232', margin: 0 }}>{formatCurrency(invoice.total_amount)}</p>
                        {invoice.balance_due > 0 && invoice.balance_due !== invoice.total_amount && (
                          <p style={{ fontSize: '12px', color: '#dc2626', margin: '2px 0 0 0' }}>Due: {formatCurrency(invoice.balance_due)}</p>
                        )}
                      </td>
                      <td style={{ padding: '16px 24px' }}>
                        <span style={{
